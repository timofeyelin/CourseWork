    ...
    public class AdminController : ControllerBase
    {
        ...
        public async Task<IActionResult> AddMeter(string accountNumber, MeterType type, string serialNumber, CancellationToken ct)
        {
            ...
            try
            {
                ...
                await _auditService.LogAsync(
                    ...
                    $"ЛС: {accountNumber}, Тип: {type}, Серийный номер: {serialNumber}",
                    ct);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> BlockUser([FromBody] BlockUserRequest request, CancellationToken ct)
        {
            ...
            try
            {
                ...
                await _auditService.LogAsync(
                    adminId,
                    request.IsActive ? "UnblockUser" : "BlockUser",
                    "User",
                    request.UserId.ToString(),
                    request.IsActive ? "Пользователь разблокирован" : "Пользователь заблокирован",
                    ct);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> ChangeRole([FromBody] ChangeRoleRequest request, CancellationToken ct)
        {
            ...
            if (Enum.TryParse<UserRole>(request.NewRole, out var role))
            {
                ...
                try
                {
                    var adminId = int.Parse(currentUserIdClaim!.Value);
                    await _auditService.LogAsync(
                        adminId,
                        "ChangeUserRole",
                        "User",
                        request.UserId.ToString(),
                        $"Роль изменена: {oldRole} -> {request.NewRole}",
                        ct);
                }
                catch (Exception ex)
                {
        ...
        public async Task<IActionResult> LinkAccount([FromBody] AdminLinkAccountRequest request, CancellationToken ct)
        {
             ...
             try
             {
                 ...
                 await _auditService.LogAsync(
                     ...
                     "Account",
                     account.AccountId.ToString(),
                     $"Привязан ЛС {request.AccountNumber} к пользователю {request.UserId}",
                     ct);
             }
             catch (Exception ex)
             {
        ...
        public async Task<IActionResult> UnlinkAccount([FromBody] AdminUnlinkAccountRequest request, CancellationToken ct)
        {
             ...
             try
             {
                 ...
                 await _auditService.LogAsync(
                     ...
                     $"Отвязан ЛС {request.AccountId} от пользователя {request.UserId}",
                     ct);
             }
             catch (Exception ex)
             {
            ...
            CancellationToken ct = default)
        {
            ...
            // Нормализуем даты в UTC
            DateTime? fromDateUtc = fromDate.HasValue 
                ? DateTime.SpecifyKind(fromDate.Value.Date, DateTimeKind.Utc) 
                : null;
            DateTime? toDateUtc = toDate.HasValue 
                ? DateTime.SpecifyKind(toDate.Value.Date, DateTimeKind.Utc) 
                : null;

            var logs = await _auditService.GetLogsAsync(page, pageSize, fromDateUtc, toDateUtc, search, ct);
            var total = await _auditService.GetTotalCountAsync(fromDateUtc, toDateUtc, search, ct);
        ...
        public async Task<IActionResult> ImportBills([FromBody] List<ImportBillDto> request, CancellationToken ct)
        {
            ...
            try 
            {
                await _auditService.LogAsync(
                    ...
                    "Bills",
                    "-",
                    $"Импорт реестра: {result}", // result содержит строку "Загружено: X..."
                    ct
                );
            }
